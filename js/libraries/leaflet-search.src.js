...
//////end DOM creations
	...
	_recordsFromLayer: function() {	//return table: key,value from layer
        ...
        function getRecordsFromLayer(someLayer){
            if(someLayer instanceof L.Control.Search.Marker) return;

			if(someLayer instanceof L.Marker || someLayer instanceof L.CircleMarker)
			{
				if(that._getPath(someLayer.options,propName))
				{
					loc = someLayer.getLatLng();
					loc.layer = someLayer;
					retRecords[ that._getPath(someLayer.options,propName) ] = loc;			
					
				}
				else if(that._getPath(someLayer.feature.properties,propName)){

					loc = someLayer.getLatLng();
					loc.layer = someLayer;
					retRecords[ that._getPath(someLayer.feature.properties,propName) ] = loc;
					
				}
				else
					throw new Error("propertyName '"+propName+"' not found in marker");
			}
            else if(someLayer.hasOwnProperty('feature'))//GeoJSON
			{
				if(someLayer.feature.properties.hasOwnProperty(propName))
				{
					loc = someLayer.getBounds().getCenter();
					loc.layer = someLayer;			
					retRecords[ someLayer.feature.properties[propName] ] = loc;
				}
				else
					throw new Error("propertyName '"+propName+"' not found in feature");
			}
			else if(someLayer instanceof L.LayerGroup)
            {
                //TODO: Optimize
                someLayer.eachLayer(function(m) {
                      getRecordsFromLayer(m);
//                    loc = m.getLatLng();
//                    loc.layer = m;
